...
class Blog extends React.Component {
  ...
  componentDidMount() {
    ...
    this.FetchByContentType(this.BlogsQuery)
      .then(this.SetBlogContent)
      .catch(console.error);
  }
              ...
              <div className={Styles.blogCard} key={element.sys.id}>
                <div className={Styles.cardImage}>
                  <Link to={`/blog/${element.fields.url}`}>
                    <img
                      src={element.fields.blogMainImage.fields.file.url}
                      alt=""
                    />
                  </Link>
                </div>
                <div className={Styles.cardCopy}>
                  <div className={Styles.cardCopyInner}>
                    <span className={Styles.dateAuthor}>
                      {DateFormat(element.fields.publishedDate, "dd mmmm yyyy")}{" "}
                      | {element.fields.author.fields.authorName}
                    </span>
                    ...
                    <p className={Styles.blogDescription}>
                      {element.fields.blogDescription}
                    </p>
                    <Link to={`/blog/${element.fields.url}`}>
                      <button className={Styles.readBtn}>READ THE POST</button>
                    </Link>
                  </div>
                </div>
              </div>
